...
void UpdateTitle() {
	...
	if (g_currentTime > g_lastTime && (g_currentTime - g_lastTime) < 0.0001) {
		return;
	}
...
HRESULT InitDevice()
{
	...
	g_hResult = D3D11CreateDeviceAndSwapChain(
		...
		D3D11_SDK_VERSION, &sd, &g_pSwapChain, &g_pDevice, &featureLevel, &g_pImmediateContext);
	...
	g_hResult = g_pDevice->CreateRenderTargetView(pBackBuffer, NULL, &g_pRenderTargetView);
    pBackBuffer->Release();
	if (FAILED(g_hResult)) {
		return g_hResult;
	}
